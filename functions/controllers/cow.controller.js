...
export const getCows = async (req, res) => {
  ...
  try {
    ...
    cowsWithLastCalving = await getLastCalvingEvents(cows, filter);
    cowsWithLastPregnant = await getLastPregnantEvents(cowsWithLastCalving);

    return res.status(200).json(getCowsResponse(cowsWithLastPregnant, foundCows.length));
  } catch (error) {
...
async function getDashboardCows(req, res) {
  ...
  try {
    ...
    if (filter === "PREGNANT") {
      const cowsWithLastPregnantEvent = await getLastPregnantEvents(foundCows);
    ...
    return res.status(200).json(getCowsResponse(slicedCows, foundCows.length));
  } catch (e) {
